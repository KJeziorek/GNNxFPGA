...
class GraphConvNorm(Module):
    def __init__(self, input_dim=1, output_dim=4, bias=False):
        super(GraphConvNorm, self).__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        self.bias = bias
        self.linear = torch.nn.Linear(input_dim + 3, output_dim, bias=bias)
...
nodes, features, edges = torch.load('graph.pt')
nodes, features, edges = nodes.to('cpu'), features.to('cpu'), edges.to('cpu')

conv = GraphConv(1, 4)
_ = conv(features, nodes, edges)
conv.eval()
print(conv(features, nodes, edges))

# mean = conv.norm.running_mean
# var = conv.norm.running_var
# std = torch.sqrt(var + conv.norm.eps)

# gamma = conv.norm.weight / std
# weights = conv.linear.weight * gamma.unsqueeze(1)
# bias = conv.norm.bias - mean * gamma

# conv_merge = GraphConvNorm(1, 4).eval()

# conv_merge.linear.weight = torch.nn.Parameter(weights)
# conv_merge.linear.bias = torch.nn.Parameter(bias)

# print(weights)
# print(bias)
# features_merge = conv_merge(features, nodes, edges)
# features_orig = conv(features, nodes, edges)
