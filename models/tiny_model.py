...
class Model(Module):
    def __init__(self, 
        ...
        self.max_pool3 = GraphPooling(pool_size=4, max_dimension=input_dimension//4, only_vertices=False, self_loop=True)

        self.conv4 = QuantGraphConv(input_dim=64, output_dim=64, bias=bias, num_bits=num_bits)
        self.relu4 = QuantReLU(num_bits=num_bits)

        out_pull = 8 if input_dimension==256 else 4
        self.out = QuantGraphPoolOut(pool_size=out_pull, max_dimension=input_dimension//8)
